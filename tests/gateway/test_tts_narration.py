...
class TestGatewayNarrationMode:
    ...
    async def test_process_narration_job_sends_chunks_in_order_and_records_success(self, runner, tmp_path, monkeypatch):
        ...
        sent_texts = [c["text"] for c in runner._tts_narration_store.list_chunks(job.job_id)]
        assert sent_texts == chunks
        assert [c["status"] for c in runner._tts_narration_store.list_chunks(job.job_id)] == ["sent"] * len(chunks)
