...
def test_edge_explicit_ogg_output_writes_mp3_then_returns_real_opus(tmp_path, monkeypatch):
    requested = tmp_path / "narration.ogg"
    generated_mp3 = tmp_path / "narration.mp3"

    def fake_convert(path: str) -> str:
        assert path == str(generated_mp3)
        requested.write_bytes(b"OggS fake")
    ...
    assert requested.read_bytes() == b"OggS fake"
    assert generated_mp3.read_bytes() == b"mp3"
    assert result["voice_compatible"] is True
    assert result["media_tag"] == f"[[audio_as_voice]]\nMEDIA:{requested}"
    convert.assert_called_once_with(str(generated_mp3))
